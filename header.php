...
$header_sticky_fixed = get_option( 'options_hc_stick' );
$header_sticky_color = get_option( 'options_hc_stickco' );
...
<style>
.media_content_item.full_content.style_side-line .full_content_inner .full_content_title:before, .masonary_grid_link.box-layout.grid_stories button.section_readmore_link:before {background-color: <?php echo $header_line_color; ?> !important;}
...
<?php }
if ( $header_sticky_fixed && $header_sticky_color ) { ?>
<style>
.page-fixed-header .header-wrap.fixed-header {background: <?php echo $header_sticky_color; ?>;}
</style>
<?php } ?>

<body <?php body_class('loading '.$masthead_style.''); ?> <?php generate_do_microdata( 'body' ); ?>>
	<div id="page_wrap"<?php if ($header_sticky_fixed) { ?> class="page-fixed-header"<?php } ?>>
	<?php
	/**
	 * wp_body_open hook.
	 *
